...
function Timeline() {
    ...
    useEffect(() => {
        ...
        const initializeGrid = async () => {
            ...
            if (isoLayerElement && isoLayerElement.instance && typeof isoLayerElement.instance.destroy === 'function') {
                try {
            ...
            if (typeof window.IsoGrid === 'function') {
                        ...
                        stackItemsAnimation: {
                            ...
                            options: function (pos, itemstotal) {
                                ...
                                if (window.dynamics) {
                                    ...
                                    console.warn('dynamics.js not loaded yet for animation options.');
                ...
                console.warn('IsoGrid constructor not found. Grid initialization skipped.');
            }
        };
    ...
    return (
        <>
                ...
                <header className="codrops-header">
                    ...
                    <div id="card-details-container">
                        {currentData.map((item, index) => (
                            <div
                                key={index}
                                className={`card-details ${activeCardIndex === index ? 'active' : ''}`}
                                id={`card-details-${index}`}
                            >
                                <h2>{item.Detail.title}</h2>
                                <p><strong>Time:</strong> {item.Detail.time}</p>
                                <p><strong>Location:</strong> {item.Detail.place}</p>
                            </div>
                        ))}
                    </div>
                </header>
            ...
            <main>
                ...
                <div ref={isoLayerRef} className="isolayer isolayer--scroll1 isolayer--shadow">
                    {/* The grid container needs a ref */}
                    <ul ref={gridContainerRef} className="grid grid--effect-flip" id="grid-container">
                        {currentData.map((item, index) => (
                            <li
                                key={index}
                            ...
                            >
                                ...
                                <a className="grid__link" href={item.link} target="_blank" rel="noopener noreferrer" onClick={(e) => e.preventDefault()}> {/* Prevent default link behavior */}
                                    <img className="grid__img layer" src={item.canvasImage} alt="Canvas Dummy" />
                                    <img className="grid__img layer" src={item.wireframeImage} alt="Wireframe Dummy" />
                                    <img className="grid__img layer" src={item.mainImage} alt={item.title} />
